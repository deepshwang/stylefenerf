...
def run(test_opts):
	...
	#test_latents = torch.rand(opts.n_images, 18, 512)
	...
	for input_batch in tqdm(dataloader):
		...
		os.makedirs(out_path_results + "/original", exist_ok=True)
		os.makedirs(out_path_results + "/edited", exist_ok=True)
		for i in range(opts.test_batch_size):
			...
			if test_opts.couple_outputs:
				...
				#torchvision.utils.save_image(result_batch[2][i], os.path.join(out_path_results + "/original", f"{im_path}.png"), normalize=True, range=(-1, 1))
				torchvision.utils.save_image(result_batch[0][i], os.path.join(out_path_results + "/edited", f"{im_path}.png"), normalize=True, range=(-1, 1))
