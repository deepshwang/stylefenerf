...
parser.add_argument("--face_angle_path", default='', type=str)
...
def run_inverse_render(opt, img_path, seg_path, face_angle):
        ...
        if not os.path.exists(checkpoint_path) or opt.load_checkpoint:
            ...
            if opt.load_checkpoint:
                meta = torch.load(os.path.join(opt.checkpoint_path, 'freq_phase_offset_{}.pt'.format(str(img_ind).zfill(5))))
                  ...
                  if i == n_iterations-1:
                        ...
                        with torch.no_grad():
                            ...
                            # for t, pitch, yaw, fov in tqdm(vis_trajectory):
                                render_options['h_mean'] = face_angle
                                # render_options['v_mean'] = pitch
                                # render_options['fov'] = fov
                                # render_options['h_stddev'] = 0
                                # render_options['v_stddev'] = 0
                                img, _, _ = generator.staged_forward_with_frequencies(w_geo_frequencies + w_geo_frequency_offsets,
                                                                                      w_app_frequencies + w_app_frequency_offsets,
                                                                                      w_geo_phase_shifts + w_geo_phase_shift_offsets,
                                                                                      w_app_phase_shifts + w_app_phase_shift_offsets, 
                                ...
                                save_image(img[:, -3:], os.path.join(save_dir, f"{img_ind}.png"), normalize=True)
...
def run_render_recon_video(opt, checkpoint_path, img_ind):
    # render video
    ...
    nrows, ncols = 1, 3
    writer = cv2.VideoWriter(os.path.join(opt.save_dir, img_ind, output_name), fourcc, 25, (256 * ncols, 256 * nrows))
...
if __name__ == "__main__":
    with open(opt.face_angle_path, 'rb') as f:
        face_angles = pickle.load(f)
    if os.path.isdir(opt.image_path) and os.path.isdir(opt.seg_path):
        img_paths, seg_paths = sorted(glob.glob(opt.image_path + '/*.png')), sorted(glob.glob(opt.seg_path + '/*.png'))
        for img_path, seg_path, face_angle in zip(img_paths, seg_paths, face_angles):
                checkpont_path = run_inverse_render(opt, img_path, seg_path, face_angle)
                #if opt.recon:
                #    img_ind = os.path.basename(img_path).split('.')[0]
                #    run_render_recon_video(opt, checkpont_path, img_ind)

    elif os.path.isfile(opt.image_path) and os.path.isfile(opt.seg_path):
        checkpoint_path = run_inverse_render(opt, opt.image_path, opt.seg_path, face_angles)
        if opt.recon:
