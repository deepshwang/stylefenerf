...
def save_image(img, save_dir, idx):
    ...
    im_save_path = os.path.join(save_dir, "e4e_inverted_images/RGB", f"{idx:05d}.png")
    os.makedirs(os.path.join(save_dir, "e4e_inverted_images/RGB"), exist_ok=True)
...
def generate_inversions(args, g, latent_codes, is_cars):
    ...
    for i in range(min(args.n_sample, len(latent_codes))):
        ...
        if is_cars:
            imgs = imgs[:, :, 64:448, :]
        save_image(imgs[0], inversions_directory_path, i)
